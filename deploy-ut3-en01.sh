#!/usr/bin/env bash
# =============================================================
# Trane UT3 — EN-0001 (embernode) deploy : join CP-02's trane-ut3
...
#      "Trane-UT3-EN-0001" (mesh/dashboard identity; gives embernet0 a
...
#   2. A one-time Flux enrollment JWT for identity "Embernode-UT3-EN01"
#      (roleAttribute 'trane-ut3'), UNLESS it is already enrolled in the
#      embernet-flux-identity volume. Provide via env or file:
#        FLUX_ENROLLMENT_JWT='<jwt>' sudo bash deploy-ut3-en01.sh
#        # or: place it at /etc/embernet/flux-en01.jwt and re-run
#
#   sudo bash deploy-ut3-en01.sh
# =============================================================
set -euo pipefail

NODE_NAME_LOWER="trane-ut3-en-0001"      # k3s node name (preserved, 4-digit)
DEVICE_NAME="Trane-UT3-EN-0001"          # embernetlite endpoint identity (mesh/dashboard)
FLUX_IDENTITY_NAME="Embernode-UT3-EN01"  # flux-edge-tunnel Ziti identity (apiserver dial)
NODE_ROLE="edge"
...
FLUX_JWT_FILE="/etc/embernet/flux-en01.jwt"
...
[[ "$EUID" -eq 0 ]] || fail "Run as root: sudo bash deploy-ut3-en01.sh"
...
# --- [2/4] Enroll AS Trane-UT3-EN-0001 --------------------------------------
enroll_endpoint() {
...
install_k3s_agent() {
  ...
  log "[4/4] EN-0001 node IP: ${node_ip}; dialing apiserver via local Flux proxy ${APISERVER_URL}"
