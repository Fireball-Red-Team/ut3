#!/usr/bin/env bash
# =============================================================
# Trane UT3 — EN-0003 (embernode) deploy : join CP-02's trane-ut3
...
#      "Trane-UT3-EN-0003" (mesh/dashboard identity; gives embernet0 a
...
#   2. A one-time Flux enrollment JWT for identity "Embernode-UT3-EN03"
#      (roleAttribute 'trane-ut3'), UNLESS it is already enrolled in the
#      embernet-flux-identity volume. Provide via env or file:
#        FLUX_ENROLLMENT_JWT='<jwt>' sudo bash deploy-ut3-en03.sh
#        # or: place it at /etc/embernet/flux-en03.jwt and re-run
#
#   sudo bash deploy-ut3-en03.sh
# =============================================================
set -euo pipefail

NODE_NAME_LOWER="trane-ut3-en-0003"      # k3s node name (preserved, 4-digit)
DEVICE_NAME="Trane-UT3-EN-0003"          # embernetlite endpoint identity (mesh/dashboard)
FLUX_IDENTITY_NAME="Embernode-UT3-EN03"  # flux-edge-tunnel Ziti identity (apiserver dial)
NODE_ROLE="edge"
...
FLUX_JWT_FILE="/etc/embernet/flux-en03.jwt"
...
[[ "$EUID" -eq 0 ]] || fail "Run as root: sudo bash deploy-ut3-en03.sh"
...
# --- [2/4] Enroll AS Trane-UT3-EN-0003 --------------------------------------
enroll_endpoint() {
...
install_k3s_agent() {
  ...
  log "[4/4] EN-0003 node IP: ${node_ip}; dialing apiserver via local Flux proxy ${APISERVER_URL}"
...
# ---------------- MAIN ----------------
log "=== Trane UT3 EN-0003 — endpoint (${DEVICE_NAME}) + Flux dial + join CP-02 as agent ==="
