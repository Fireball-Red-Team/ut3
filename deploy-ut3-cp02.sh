#!/bin/bash
# =============================================================
# EmberNet UT3 — Control Plane 02 Deployment Script
# Node: Trane-UT3-CP-02  |  embernet0 IP: assigned by embernetlite at enrollment
# OS:   Ubuntu 24.04 Server (x86_64)
# Run as root: sudo bash deploy-ut3-cp02.sh
#
# CP-02 is the SECOND control plane of CP-01's HA K3s cluster.
# It does NOT seed its own etcd — it joins CP-01 via:
...
#
# CP-01 already owns the Rancher import for this cluster; CP-02
# inherits cattle-cluster-agent through cluster membership. There
# is NO Rancher-registration step on CP-02 — adding one would
# create a duplicate registration. See ARCHITECTURE.md § Rancher
# import ("Don't try to import CP-02 separately").
#
...
# (K3s server HA-join on embernet0).
#
...
#   - Ignition Edge 8.3.6 (host podman, host:8088)                [Phase 1]
#       NOTE: connects to CP-01's PostgreSQL at ${CP01_EMBERNET_IP}:5432
#       over embernet0. Postgres is NOT installed on CP-02 — CP-01 owns
#       the DB for the whole Trane UT3 fleet.
#   - CODESYS Control SL 4.20 (host podman, host:1217)            [Phase 1]
#   - K3s server v1.34.5+k3s1 (host install, --server HA join to CP-01) [Phase 2]
#
# PRE-REQUISITE: /etc/embernet/k3s-token must exist on CP-02 BEFORE
...
#   CP01_EMBERNET_IP=<cp01-embernet0-ip>  sudo -E bash trane/deploy-ut3-cp02.sh
# preflight_checks() fails hard (Phase 2) if the token file is missing.
#
# Reference:
#   - .agent/EXECUTION_PLAN.md § Phase 3
#   - .agent/ARCHITECTURE.md  (mandatory labels, "Don't try to import CP-02 separately")
...
NODE_NAME="${NODE_NAME:-Trane-UT3-CP-02}"
NODE_NAME_LOWER="$(printf '%s' "${NODE_NAME}" | tr '[:upper:]' '[:lower:]')"
# CP-02's embernet0 IPv4 is assigned by the embernetlite provisioner at
# enrollment time. Phase 2 detects it via `ip -4 addr show embernet0`
# and binds K3s to that address.
EMBERNET_IFACE="embernet0"

# CP-01's embernet0 IP — CP-02 joins CP-01's K3s API + connects to CP-01's
# Postgres at this address over embernet0. Default 100.64.1.1 matches the
# legacy fleet WG numbering and is what the embernetlite provisioner
# assigns to CP-01 in the typical case. Override if your tenant got a
# different address:
#   CP01_EMBERNET_IP=<addr>  sudo -E bash trane/deploy-ut3-cp02.sh
CP01_EMBERNET_IP="${CP01_EMBERNET_IP:-100.64.0.38}"
CP01_PG_USER="ut3"
CP01_PG_DB="ignition_tranetech"

# Ignition Edge — Embernet-owned multi-arch build from
# Embernet-ai/ignition-packages (CI publishes on every push to main).
# CP-02 hosts EDGE only (Cloud lives on CP-01). Default ports 8088/8043/8060
# on the host — no co-resident remap needed since Cloud isn't here.
# To temporarily fall back to IA's official multi-arch image, swap
# IGNITION_EDGE_IMAGE to docker.io/inductiveautomation/ignition:8.3.6.
...
# K3s — host install, HA-joining CP-01's existing cluster (NOT
# initializing a new etcd). Token is provisioned out-of-band by copying
# /etc/embernet/k3s-token from CP-01 BEFORE running this script.
K3S_VERSION="v1.34.5+k3s1"
K3S_INSTALL_NAME="embernet-server"
K3S_TOKEN_FILE="/etc/embernet/k3s-token"

# =============================================================
# HELPERS — structured logging (matches CP-01 / CP-02 conventions)
# =============================================================
...
# after a settle. Pattern lifted verbatim from CP-01 / CP-02.
quadlet_restart() {
  local svc="$1"
...
# ─── [2/10] Firewall (UFW) ───────────────────────────────────
configure_firewall() {
  log "[2/10] Configuring UFW & Network Routing..."
  ...
  # K3s API server — CP-02 also serves the API as a second control plane
  ufw allow 6443/tcp || true

  # Ignition Edge (host-side ports 8088/8043/8060)
  ufw allow 8088/tcp || true
  ufw allow 8043/tcp || true
  ufw allow 8060/udp || true

  # CODESYS gateway
  ufw allow 1217/tcp || true

  # embernet0 (embernetlite-managed WireGuard) is the trusted overlay —
  # K3s API, flannel, kubelet, inter-controlplane etcd, and CP-01
  # Postgres traffic all ride on it.
  ...
  log "[2/10] Firewall configured"
}

# ─── [3/10] Crash-reboot hardening ───────────────────────────
...
configure_crash_reboot() {
  log "[3/10] Configuring systemd PID 1 crash-reboot hardening..."
...
# embernet0 in the host netns and Phase 2 of this script binds K3s
# to it.)

# ─── [4/10] Ignition Edge (host podman, host:8088) ───────────
# CP-02 hosts EDGE only (same as CP-02). The gateway connects to
# CP-01's PostgreSQL at ${CP01_EMBERNET_IP}:5432 over embernet0 — Postgres is NOT
# installed locally; CP-01 owns the DB for the whole Trane UT3 fleet.
#
# The DB connection itself is configured via the Ignition Gateway UI
# (Config → Databases → Connections) AFTER first start; this script
# only ensures the gateway is up and the host firewall permits the
# outbound embernet0 traffic to ${CP01_EMBERNET_IP}:5432. The summary block at the
# bottom prints the JDBC URL for the operator to paste in.
install_ignition_edge() {
  log "[4/10] Starting Ignition Edge ${IGNITION_EDGE_VERSION}..."

  # Tear down a prior plain "ignition" container from the Standard era
  # so its port 8088 binding doesn't collide with the new Edge container.
  if podman container exists ignition 2>/dev/null; then
    log "Removing prior 'ignition' (Standard era) container — replaced by ignition-edge"
    podman rm -f ignition >/dev/null 2>&1 || true
  fi
  if [[ -f /etc/systemd/system/container-ignition.service ]]; then
    systemctl disable --now container-ignition.service 2>/dev/null || true
    rm -f /etc/systemd/system/container-ignition.service
    systemctl daemon-reload 2>/dev/null || true
  fi

  # Tear down any legacy ignition-cloud artifacts from when CP-02 may
  # have co-hosted Cloud during earlier deploy iterations (Cloud is on
  # CP-01 only in the target architecture).
  if podman container exists ignition-cloud 2>/dev/null; then
    log "Removing legacy ignition-cloud container (Cloud is on CP-01 only post-rewrite)"
    podman rm -f ignition-cloud >/dev/null 2>&1 || true
  fi
  if [[ -f /etc/systemd/system/container-ignition-cloud.service ]]; then
    systemctl disable --now container-ignition-cloud.service 2>/dev/null || true
    rm -f /etc/systemd/system/container-ignition-cloud.service
    systemctl daemon-reload 2>/dev/null || true
  fi

  if container_already_running ignition-edge; then
    log "[4/10] Ignition Edge already running — skipping"
    return 0
  fi

  # Tear down any orphan Quadlets from prior iterations
  for stale in ut3-ignition-edge.container ut3-ignition-standard.container ut3-ignition-cloud.container; do
    if [[ -f /etc/containers/systemd/${stale} ]]; then
      rm -f "/etc/containers/systemd/${stale}"
      systemctl daemon-reload 2>/dev/null || true
    fi
  done

  # ---- Seed the data dir from the image (FIRST RUN ONLY) ----
  # VERIFIED: an empty bind-mount at /usr/local/bin/ignition/data
  # MASKS the in-image data tree (gateway.xml_clean, init.properties,
  # db/, local/ — IA forum thread 111259, IA staff Kevin Collins).
  # ignition.sh then can't find init.properties / gateway.xml_clean and
  # fails with errors that look like permission failures but are
  # actually "files don't exist" errors. The Embernet Containerfile
  # doesn't ship IA's auto-seed entrypoint, so seed the host bind-mount
  # once before the live container starts.
  if [[ ! -f /opt/embernet/ignition-edge/data/.embernet-seeded ]]; then
    log "First-run: seeding /opt/embernet/ignition-edge/data from image..."
    # Stream full podman pull output so layer-by-layer download progress
    # is visible to the operator.
    podman pull "${IGNITION_EDGE_IMAGE}" || true
    if podman run --rm \
         --network=none \
         --entrypoint /bin/bash \
         -v /opt/embernet/ignition-edge/data:/seed \
         "${IGNITION_EDGE_IMAGE}" \
         -c 'cp -an /usr/local/bin/ignition/data/. /seed/ 2>/dev/null && chmod +x /seed/*.sh /seed/**/*.sh 2>/dev/null; touch /seed/.embernet-seeded'; then
      find /opt/embernet/ignition-edge/data -name '*.sh' -exec chmod +x {} + 2>/dev/null || true
      log "  seeded $(du -sh /opt/embernet/ignition-edge/data 2>/dev/null | cut -f1)"
    else
      warn "Seed step failed — first start may crash. Manually seed with:"
      warn "  podman run --rm --network=none -v /opt/embernet/ignition-edge/data:/seed --entrypoint /bin/bash ${IGNITION_EDGE_IMAGE} -c 'cp -an /usr/local/bin/ignition/data/. /seed/'"
    fi
  else
    log "Data dir already seeded (.embernet-seeded marker present) — preserving operator state"
  fi

  # The ghcr.io/embernet-ai/ignition-edge image ships an ignition.sh that
  # already has `#RUN_AS_USER=` commented (matches official IA Dockerfile),
  # so the wrapper runs as root inside the container — no patching needed
  # (Cloud's image has the RUN_AS_USER=ignition mis-build; Edge does not).
  podman run -d \
    --name ignition-edge \
    --restart=always \
    --network=host \
    ...
    -v /opt/embernet/ignition-edge/data:/usr/local/bin/ignition/data \
    "${IGNITION_EDGE_IMAGE}"
  ...
  warn "[4/10] Ignition Edge container started but not responding after 60s — check: podman logs ignition-edge"
}

# ─── [5/10] CODESYS Control SL 4.20 (host podman, host:1217) ─
...
# CP-01's install_codesys().
install_codesys() {
  log "[5/10] Starting CODESYS Control SL ${CODESYS_VERSION}..."
  ...
  if container_already_running codesys; then
    ...
    if [[ "${pid1}" == *"sleep"* ]]; then
      ...
      log "[5/10] Codesys already running (PID 1: ${pid1}) — skipping rebuild"
      return 0
    fi
  fi
  ...
  if ! podman image exists "${CODESYS_IMAGE}" 2>/dev/null; then
    ...
    if ! wget -q --show-progress -O "${BUILD_DIR}/codesys.pkg" "${CODESYS_URL}"; then
      warn "[5/10] Codesys download failed — skipping"
      rm -rf "${BUILD_DIR}"
      return 0
    fi
    ...
    if [[ ${pkg_size} -lt 1024 ]]; then
      warn "[5/10] Codesys download truncated (${pkg_size} bytes) — skipping"
      rm -rf "${BUILD_DIR}"
      return 0
    fi
...
DOCKERFILE
    ...
    if ! podman build -t "${CODESYS_IMAGE}" "${BUILD_DIR}"; then
      warn "[5/10] Codesys container build failed — skipping"
      rm -rf "${BUILD_DIR}"
      return 0
    fi
    rm -rf "${BUILD_DIR}"
  ...
  log "[5/10] Codesys: $(podman ps --filter name=codesys --format '{{.Status}}' 2>/dev/null || echo 'starting')"
}

# ─── [6/10] embernetlite (EmbernetEndpoint-Linux, Quadlet) ───
...
#     embernet0 detected with an IPv4 → K3s HA-joins CP-01 over it.
...
install_k3s_server() {
  ...
  #
  #   HOST_TLS_SAN=<addr>  sudo -E bash trane/deploy-ut3-cp02.sh
  local HOST_LAN_IP
...
# (No Rancher-registration step on CP-02 — CP-01's Rancher import
# covers the whole HA cluster. Adding one here would create a duplicate
# Rancher registration for the same cluster.)

# ─── [9/10] verify_deployment — the gate ─────────────────────
# Each check prints `[PASS]` or `[FAIL]` followed by the check name.
# Returns 0 if ALL pass, exits 1 otherwise. This is the Phase 3
# exit-criteria gate. Mirrors CP-02 verify_deployment with two
# delta:
#   - etcd member count == 2 (CP-01 + CP-02), warn-skip if CP-01
#     unreachable at verify time
#   - NO cattle-cluster-agent check (the pod runs on CP-01; CP-02
#     inherits cluster membership without its own import)
verify_deployment() {
  ...
  if [[ -s /var/lib/embernet/auth.token ]]; then
    ...
    if podman exec systemd-embernet curl -sf -m 5 \
      ...
      _check "embernetlite API /api/v1/health responding — FAILED (pre-enrollment? re-run \`sudo bash trane/deploy-ut3-cp02.sh\` and complete the AAD device-code flow when prompted)" FAIL
    fi
  else
...
print_summary() {
  ...
  for svc in embernet container-ignition-edge container-codesys "k3s-${K3S_INSTALL_NAME}"; do
  ...
  echo "  K3s API (CP-01 HA pair):   https://${CP01_EMBERNET_IP}:6443  (joined as second control plane)"
  echo "  K3s API (local):           https://${EMBERNET_IP:-<embernet0 not up>}:6443     (CP-02's own API endpoint)"
  echo "  Ignition Edge UI:          http://localhost:8088   (admin / ${IGNITION_ADMIN_PASSWORD})"
  echo "  Ignition Edge DB target:   jdbc:postgresql://${CP01_EMBERNET_IP}:5432/${CP01_PG_DB}"
  echo "                             (external — CP-01 owns Postgres at ${CP01_EMBERNET_IP}:5432;"
  echo "                              configure via Ignition Gateway UI: Config → Databases → Connections)"
  echo "  CODESYS gateway:           localhost:1217"
  echo "  Dashboard:                 https://dashboard.embernet.ai  (visible via CP-01's Rancher import)"
  echo ""
  echo "  --- HA cluster membership ---"
  /usr/local/bin/k3s kubectl get nodes -o wide 2>/dev/null \
  ...
  echo "  Re-run `sudo bash trane/deploy-ut3-cp02.sh` — the script will"
...
echo ""
echo "============================================================"
echo "  EmberNet UT3 — Control Plane 02 Deployment (HA join)"
echo "  Node: ${NODE_NAME}"
...
# Phase 2 — K3s HA-join CP-01 on embernet0:
