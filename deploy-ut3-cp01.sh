...
install_embernetlite() {
  ...
  # Explicit pull BEFORE the Quadlet drops. Two reasons:
  #   1. systemd-restart-on-failure won't burn the StartLimitBurst budget
  #      if the image is already local.
  #   2. We removed `Pull=newer` from the Quadlet body for podman 4.9
  #      generator compatibility (key not reliably supported below
  #      podman 5.x); the explicit pull replaces it.
  log "Pulling ${EMBERNET_IMAGE}..."
  podman pull "${EMBERNET_IMAGE}" || warn "pre-pull failed — Quadlet will retry at start"

  # Drop the Quadlet. Bare-bones spec — only keys supported by every
  # Quadlet generator from podman 4.5 forward (Ubuntu 24.04 ships 4.9.3;
  # Tumbleweed / MicroOS run 5.x). Specifically REMOVED for portability:
  #   - Documentation=  (decorative; some 4.9 generators are picky)
  #   - DropCapability=all  (Capabilities= behavior diverges 4.9 ↔ 5.x;
  #                          dropping it leaves the default cap set
  #                          minus the host's no-new-privs — Add*=
  #                          re-grants what embernetlite actually needs)
  #   - Pull=newer  (handled by the explicit `podman pull` above)
  #   - HealthCmd=/HealthStartPeriod=  (HealthStartPeriod= not reliable
  #                                     on 4.9; the daemon's own /api/v1
  #                                     /health endpoint is the canonical
  #                                     liveness, surfaced via
  #                                     verify_deployment).
  cat > /etc/containers/systemd/embernet.container <<QUADLET
# Quadlet — embernetlite v0.0.29 (bare-bones, podman 4.5+ compatible)
...
# Caps embernetlite needs for wireguard / openvpn / tun.
...
QUADLET
  ...
  # Diagnostic: surface the Quadlet generator's own opinion BEFORE we
  # tell systemd to restart. If the generator rejected a key, this is
  # how we find out (the lesson from the 10.0.1.92 PullPolicy= debacle).
  local generator=""
  for path in /usr/lib/systemd/system-generators/podman-system-generator \
    ...
    [[ -x "${path}" ]] && { generator="${path}"; break; }
  done
  if [[ -n "${generator}" ]]; then
    log "Validating Quadlet via ${generator##*/} --dryrun..."
    local gen_out
    gen_out=$("${generator}" --dryrun 2>&1 | grep -iE 'embernet|error|unsupported' | head -20 || true)
    if [[ -n "${gen_out}" ]]; then
      printf '    %s\n' "${gen_out}"
    fi
    if echo "${gen_out}" | grep -qiE 'unsupported|error'; then
      fail "Quadlet generator rejected the .container file (see output above). \
The .service unit will NOT be created; systemctl restart will fail. Inspect the rejected key + fix the inline Quadlet body in install_embernetlite()."
    fi
  else
    warn "No podman-system-generator found at the expected paths — can't pre-validate Quadlet. Continuing."
  fi
